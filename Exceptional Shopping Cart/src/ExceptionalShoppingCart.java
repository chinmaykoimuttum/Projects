...
/**
 * The class ExceptionalShoppingCart executes the methods listed in 
 * ExceptionalShoppingCart which deal with editing and working with 
 * marketItems array.
 * The class is capable of adding items to cart and expanding capacity 
 * of cart, 
 * The class can also return a summary of the cart and calculates occurrences
 * of items and total price of cart.
 * The class can also use item name, price, and id to search through 
 * marketItems array and look up Id and name of item.
 * The class can also save cart summary to file, parse the contents of 
 * the lines of the cart summary, and load the cart summary from the
 * file.
 * 
...
public class ExceptionalShoppingCart {
  ...
  public static String lookupProductByName(String name) 
    ...
    for (int i = 0; i < marketItems.length; i++) {
      ...
      if (i == marketItems.length - 1) {
        throw new NoSuchElementException("Error found: No match found for name");
      }
    }
    return s;
  }
  ...
  public static String lookupProductById(int key)
    ...
    int digits = String.valueOf(key).length();
    // throws IllegalArgumentException with descriptive error message if key is not a 4-digits int
    if (digits < 4 || digits > 4) {
      throw new IllegalArgumentException("Error found: Key is not 4 digits long");
    }
  ...
  public static void addItemToMarketCatalog(String id, String name, String price)
    ...
    // throws IllegalArgumentException with descriptive error message if id not parsable 
    try {
      Integer.parseInt(id);
    } catch (NumberFormatException e) {
      throw new IllegalArgumentException("addItemToMarketCatalog) Error found: ID is not parsable");
    } catch (Exception e) {
      System.out.println("addItemToMarketCatalog) Error found: ID has some error");
    }
    // throws IllegalArgumentException with descriptive error message if id not parsable to 4-digits
    int parseTestID = String.valueOf(Integer.parseInt(id)).length();
    if (parseTestID < 4 || parseTestID > 4) {
      throw new IllegalArgumentException(
          "addItemToMarketCatalog) Error found: ID not parsable to 4 digits");
    }
    // throws IllegalArgumentException with descriptive error message if name is null or empty
    if (name == null || name.equals("")) {
      throw new IllegalArgumentException(
          "addItemToMarketCatalog) Error found: name is empty or null");
    }
    // throws IllegalArgumentException with error message if price doesn't start with $
    try {
      if (!(price.substring(0, 1).equals("$"))) {
        // System.out.println(price.substring(0,1));
        throw new IllegalArgumentException(
            "addItemToMarketCatalog) Error found: Price does not start with $");
      }
      // throws IllegalArgumentException with error message if price isn't positive
      Double.parseDouble(price.substring(1));
      if (Double.parseDouble(price.substring(1)) <= 0.0) {
        throw new IllegalArgumentException(
            "addItemToMarketCatalog) Error found: Price is not positive");
      }
    } catch (NumberFormatException e) {
      throw new IllegalArgumentException(
          "addItemToMarketCatalog) Error found: Price is not parsable to double");
    } catch (Exception e) {
      System.out.println("addItemToMarketCatalog) Error found: Price has some error");
      System.out.println(e.getMessage());
    }
    // throws IllegalArgumentException with error message if catalog is full
    int next = indexOfInsertionPos();
    if (next == marketItems.length) {
      System.out.println("Full catalog! No further item can be added!");
      String[][] expandedMarketItems = new String[2*(marketItems.length)][2*(marketItems.length)];
      for(int i = 0; i < marketItems.length; i++)
        expandedMarketItems[i] = marketItems[i];
    } else {
  ...
  public static double getProductPrice(String name) throws NoSuchElementException {
    ...
    for (int i = 0; i < marketItems.length; i++) {
      ...
      // throws NoSuchElementException with descriptive error message if price not found
      if (i == marketItems.length - 1) {
        throw new NoSuchElementException("getProductPrice) Error found: No match found for name");
      }
    }
    return price;
  }
  ...
  public static int addItemToCart(String item, String[] cart, int size)
    ...
    // throws IllegalArgumentException with descriptive error message if size is less than zero
    if (size < 0) {
      throw new IllegalArgumentException("addItemToCart) Error found: Size is less than zero");
    }
    // throws IllegalStateException with descriptive error message if this cart is full
    if (cart.length == size) {
      throw new IllegalStateException("addItemToCart) Error found: Cart is full");
    }
    cart[size] = item;
    size++;
    return size;
  ...
  public static int nbOccurrences(String item, String[] cart, int size)
    ...
    // throws IllegalArgumentException with descriptive error message if size is less than zero
    if (size < 0) {
      throw new IllegalArgumentException("nbOccurrences) Error found: Size is less than zero");
    }
    int count = 0;
  ...
  public static boolean contains(String item, String[] cart, int size)
    ...
    // throws IllegalArgumentException with descriptive error message if size is less than zero
    if (size < 0) {
      throw new IllegalArgumentException("contains) Error found: Size is less than zero");
    }
    for (int i = 0; i < size; i++) {
      if (cart[i].equals(item)) {
        return true;
  ...
  public static int removeItem(String[] cart, String item, int size)
    ...
    // throws IllegalArgumentException with descriptive error message if size is less than zero
    if (size < 0) {
      throw new IllegalArgumentException("removeItem) Error found: Size is less than zero");
    }
    for (int i = 0; i < size; i++) {
      if (cart[i].equals(item)) {
        cart[i] = cart[size - 1];
      ...
      // throws NoSuchElementException with descriptive error message if item not found in the cart
      if (i == size - 1) {
        throw new NoSuchElementException("removeItem) Error found: No match found for size");
      }
    }
    return size;
  }
  ...
  public static int emptyCart(String[] cart, int size)
    ...
    // throws IllegalArgumentException with descriptive error message if size is less than zero
    if (size < 0) {
      throw new IllegalArgumentException("emptyCart) Error found: Size is less than 0");
    }
    // throws NullPointerException with descriptive error message if cart is null
    if (size == 0) {
      throw new NullPointerException("emptyCart) Error found: Cart is null");
    }
  ...
  public static double checkout(String[] cart, int size) throws IllegalArgumentException {
    // throws IllegalArgumentException with descriptive error message if size is less than zero
    if (size < 0) {
      throw new IllegalArgumentException("checkout) Error found: Size is less than 0");
    }
    double total = 0.0;
    for (int i = 0; i < size; i++) {
  ...
  public static String getCartSummary(String[] cart, int size) throws IllegalArgumentException {
    // throws IllegalArgumentException with descriptive error message if size is less than zero
    if (size < 0) {
      throw new IllegalArgumentException("getCartSummary) Error found: Size is less than 0");
    }
    String s = "";
    for (int i = 0; i < size; i++) {
  ...
  public static void saveCartSummary(String[] cart, int size, File file)
    ...
    // throws IllegalArgumentException with descriptive error message if size is less than zero
    if (size < 0) {
      throw new IllegalArgumentException("saveCartSummary) Error found: Size is less than 0");
    }
    FileWriter fileWriter = new FileWriter(file);
    PrintWriter printWriter = new PrintWriter(fileWriter);
    printWriter.print(getCartSummary(cart, size));
    printWriter.close();
  }
  ...
  protected static int parseCartSummaryLine(String line, String[] cart, int size)
      throws DataFormatException, IllegalStateException, NoSuchElementException,
      IllegalArgumentException {
    // throws DataFormatException with descriptive error message if line is null
    if (line == null) {
      throw new DataFormatException("parseCartSummaryLine) Error found: "
          + "Line is null");
    }
    // throws DataFormatExceptions with descriptive error message if parsedValue is not 
    // formatted properly
    String[] parsedValue = line.trim().split(" ");
    ...
    if (parsedValue.length != 4) {
      throw new DataFormatException("parseCartSummaryLine) Error found: "
          + "formatting issue for length of parsedValue");
    }
    if (!parsedValue[0].equals("(") || !parsedValue[2].equals(")")) {
      throw new DataFormatException("parseCartSummaryLine) Error found: "
          + "parantheses are wrongly formatted");
    }
    // throws DataFormatException with descriptive error message if 
    //nbOccurrences not parsable to a positive integer less or equal to 10)
    try {
      int nb = Integer.parseInt(parsedValue[1]);
      if (nb < 0 || nb > 10) {
        throw new DataFormatException(
            "parseCartSummaryLine) Error found: nbOccurrences is not positive or equal to 10");
      }
    } catch (NumberFormatException e) {
      System.out.println(e.getMessage());
      throw new IllegalArgumentException(
          "parseCartSummaryLine) Error found: Cannot parse nbOccurrences");
    }
    // throws DataFormatExceptions with descriptive error message if parsedValue is not 
    // formatted properly
    try {
      String name = parsedValue[3];
      lookupProductByName(name);
    } catch (NoSuchElementException e) {
      throw new IllegalArgumentException("parseCartSummaryLine) "
          + "Error found: formatting issue with itemName");
    }
    int nb = Integer.parseInt(parsedValue[1]);
    String name = parsedValue[3];
    for (int i = 0; i < marketItems.length; i++) {
      if (marketItems[i] != null) {
        ...
        if (i == marketItems.length - 1) {
          throw new IllegalArgumentException("parseCartSummaryLine) "
              + "Error found: itemName not found");
        }
      }
    }
    // appends item to cart if parsedValue is formatted correctly
    if (parsedValue[0].equals("(") && parsedValue[2].equals(")")) {
      addItemToCart(name, cart, size);
    }
    try {
      for (int i = 0; i < nb; i++) {
        addItemToCart(name, cart, size + i);
      }
      // throws IllegalStateException with descriptive error message if cart reaches its capacity
    } catch (IllegalStateException e) {
      throw new IllegalStateException("parseCartSummaryLine) Error found: Cart reached capacity");
    }
    return size + nb; // default return statement added to avoid compiler errors
  }
  ...
  public static int loadCartSummary(File file, String[] cart, int size)
    ...
    //throws IllegalArgumentException with descriptive error message if size is less than zero
    if (size < 0) {
      throw new IllegalArgumentException("loadCartSummary) Error found: Size is less than 0");
    }
    Scanner scnr = null;
    //reads line with scanner from file and updates size
    try {
      scnr = new Scanner(file);
      while (scnr.hasNextLine()) {
        String currentLine = scnr.nextLine();
        try {
          size = parseCartSummaryLine(currentLine, cart, size);
        } catch (IllegalArgumentException e) {
        } catch (DataFormatException e) {
        } catch (IllegalStateException e) {
          throw new IllegalStateException("loadCartSummary) "
              + "Error found: capacity is full");
        }
      }
    }
    catch (IOException e) {
